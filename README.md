...
**NitoNode-Explorer** est un script d'auto-installation conçu pour déployer un nœud complet NitoCoin ([https://github.com/NitoNetwork/Nito-core](https://github.com/NitoNetwork/Nito-core)) et un explorateur de blockchain eIquidus ([https://github.com/team-exor/eiquidus](https://github.com/team-exor/eiquidus)) sur un serveur Linux (Ubuntu recommandé). Ce script automatise l'installation, la configuration, et la synchronisation des deux composants, te permettant d'avoir un nœud opérationnel et un explorateur web accessible en quelques étapes simples.
...
- Option pour choisir le répertoire d'installation (par défaut : `/root`).
...
3. **Lancer l'installation** :  
   ...
   - Le script te posera cinq questions :
     ...
     - **Répertoire d'installation** : Entre le répertoire où installer l'explorateur (ex. : `/root` ou `/var/www`, appuie sur Entrée pour utiliser `/root` par défaut).
   ...
   - Téléchargement et configuration du nœud NitoCoin (version 2.0.1) avec les identifiants choisis, dans le répertoire spécifié.
   - Démarrage du nœud et début de la synchronisation de la blockchain.
   - Installation de l'explorateur eIquidus dans le répertoire spécifié, avec MongoDB ([https://www.mongodb.com/](https://www.mongodb.com/)) pour la base de données.
   - Configuration de Nginx ([https://nginx.org/](https://nginx.org/)) comme reverse proxy avec un certificat SSL via Certbot ([https://certbot.eff.org/](https://certbot.eff.org/)), adapté au domaine saisi.
   - Ouverture des ports nécessaires (SSH, 8820, 80/443, 27017, et le port RPC choisi) via UFW.
   - Téléchargement des images personnalisées (logo, favicons, etc.) depuis le dépôt GitHub.
   - Placement des favicons (`favicon-32.png`, `favicon-128.png`, `favicon-180.png`, `favicon-192.png`) dans `[répertoire choisi]/explorer/public/`.
   - Placement des autres images (`logo.png`, `header-logo.png`, `page-title-img.png`, `external.png`, `coingecko.png`) dans `[répertoire choisi]/explorer/public/img/`.
   - Téléchargement et configuration de `settings.json` avec les identifiants et les chemins SSL adaptés au domaine saisi.
   - Lancement de l'explorateur avec PM2 ([https://pm2.keymetrics.io/](https://pm2.keymetrics.io/)) et synchronisation automatique de la blockchain.
   - Configuration de PM2 et MongoDB pour redémarrer automatiquement en cas de redémarrage du serveur.
   - Configuration d'un cron pour synchroniser l'explorateur toutes les minutes.

4. **Attendre la fin de l'installation** :
   - L'installation prend environ 10 à 20 minutes, selon la vitesse de ton serveur et de ta connexion Internet.
   - À la fin, un message s'affichera avec l'URL de ton explorateur (ex. : `https://nito-explorer.nitopool.fr`), les identifiants que tu as choisis, et le répertoire d'installation.
...
- **Si le favicon ne s'affiche pas correctement** :
  ...
  - Vérifie que les fichiers `favicon-32.png`, `favicon-128.png`, `favicon-180.png`, et `favicon-192.png` sont bien présents dans `<répertoire choisi>/explorer/public/` :  
    ```bash
    ls <répertoire choisi>/explorer/public/
    ```
